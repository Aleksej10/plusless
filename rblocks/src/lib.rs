...
impl Block {
  fn new(blk: Blk, should_update: Arc<RwLock<bool>>) -> Block {
    Block { blk, res: Arc::new(RwLock::new(String::from(""))), should_update }
  }
...
impl Blocks {
  fn new(blocks: Vec<Blk>) -> Blocks {
    ...
    let data = blocks.into_iter().map(|blk| {
      Block::new(blk, Arc::clone(&should_update))
    }).collect();

    Blocks { data, should_update }
  }
...
pub fn initialize() -> Result<(Blocks, Signals), &'static str> {
  ...
  for blk in BLOCKS.iter() {
    ...
    let ico = if blk.ico.is_empty() { byte } else { format!("{}{} ", byte, blk.ico.trim()) };

    blocks.push(Blk { sig, t: blk.t, ico, cmd: blk.cmd });
...
fn parse_signal(iter: &mut signal_hook::iterator::Forever<signal_hook::iterator::exfiltrator::SignalOnly>) -> Option<Sig> {
  let blk = iter.next()? - SIGRTMIN;
  if blk < 7 { return None; }
  ...
  if btn < 7 {
    return Some(Sig { blk, btn: Some(btn) });
  }
  None
}
